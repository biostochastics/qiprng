...
generate_html_header <- function(report) {
  c(
    ...
    sprintf(
      "      <p>Validation Level: <strong>%s</strong></p>",
      tools::toTitleCase(report$level)
    ),
    sprintf(
      "      <p>Execution Time: <strong>%s</strong></p>",
...
generate_html_summary <- function(report) {
  ...
  c(
    ...
    sprintf(
      '        <div class="progress-fill" style="width: %.1f%%">%.1f%% Pass Rate</div>',
      pass_rate, pass_rate
    ),
    "      </div>",
    "    </section>"
...
generate_html_edge_cases <- function(edge_results) {
  ...
  if (!is.null(edge_results$total_tests)) {
    ...
    html <- c(
      ...
      sprintf(
        "        <p>Total edge case tests: <strong>%d</strong></p>",
        edge_results$total_tests
      ),
      sprintf(
        '        <p>Passed: <strong class="pass">%d</strong> (%.1f%%)</p>',
        edge_results$passed, pass_rate
      ),
      sprintf(
        '        <p>Failed: <strong class="fail">%d</strong></p>',
        edge_results$failed
      ),
      "      </div>"
    )
...
generate_rmarkdown_content <- function(report, include_visualizations) {
  c(
    ...
    "",
    sprintf(
      "This validation was performed at level **%s** and took **%s** to complete.",
