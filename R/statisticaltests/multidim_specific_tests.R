...
uniformity_test_2d <- function(points, grid_size = NULL, 
  ...
  result <- list(
    ...
    details = sprintf("Grid: %dx%d, Empty: %.1f%%, CV: %.3f", 
                     grid_size, grid_size, 
...
scatter_test_3d <- function(points, method = "triplet") {
  ...
  if (method == "triplet") {
    ...
    result <- list(
      ...
      details = sprintf("Mean triplet area: %.4f (expected: %.4f)", 
    ...
    result <- list(
      ...
      details = sprintf("Chi-square: %.3f, Min count: %d, Max count: %d",
...
minimum_distance_test <- function(points, theoretical_dist = "poisson") {
  ...
  if (theoretical_dist == "poisson") {
    ...
    result <- list(
      ...
      details = sprintf("KS test vs %s: D = %.4f", 
    ...
    p_value <- 2 * min(mean(boot_stats <= observed_mean), 
    ...
    result <- list(
      ...
      details = sprintf("Bootstrap test: Observed mean = %.4f, Expected = %.4f",
...
convex_hull_volume_test <- function(points, n_simulations = 1000) {
  ...
  # Calculate p-value
  p_value <- 2 * min(mean(sim_volumes <= observed_volume),
  ...
  result <- list(
    ...
    details = sprintf("Volume: %.4f (expected: %.4f ± %.4f)",
  ...
  if (max_dim >= 2) {
    ...
    if (n - lag >= 10) {
      ...
      "FAIL"
        ...
        details = sprintf("Distance lag %d correlation: %.4f", 
...
calculate_morans_i_2d <- function(cell_counts, grid_size) {
  ...
  return(list(statistic = I, expected = E_I, variance = V_I, 
