...
visualize_classical_tests <- function(suite, x, collection_times, expected_time) {
  ...
  # Count frequencies
  hand_counts <- table(factor(hand_types, levels = c(
    "all_different", "one_pair", "two_pair",
  ...
  expected_probs <- c(
    p_all_diff, p_one_pair, p_two_pair, p_three,
  ...
  hand_data_long <- reshape2::melt(hand_data,
    id.vars = "HandType",
    variable.name = "Type",
    value.name = "Count"
  )
  ...
  collision_data_long <- reshape2::melt(collision_data,
    id.vars = "Collisions",
    variable.name = "Type",
    value.name = "Count"
  )
  ...
  p3 <- ggplot2::ggplot(
    ...
    ggplot2::labs(
      ...
      subtitle = paste(
        "Distribution of collisions with expected Poisson(λ =",
        round(lambda, 2), ")"
      ),
