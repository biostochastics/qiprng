...
generate_all_visualizations <- function(
  ...
  if (length(missing_packages) > 0) {
    warning(sprintf("Missing packages for full visualization: %s", 
...
create_test_result_heatmap <- function(suite, output_dir) {
  ...
  ggsave(
    ...
    p_category, width = 10, height = 8, dpi = 300
  )
...
create_pvalue_distribution_plots <- function(suite, output_dir) {
  ...
  ggsave(
    ...
    p_category, width = 14, height = 10, dpi = 300
  )
...
create_statistic_distribution_plots <- function(suite, internal, output_dir) {
  ...
  for (name in names(plot_list)) {
    ggsave(
      ...
      plot_list[[name]], width = 12, height = 8, dpi = 300
    )
  }
  ...
  if (length(plot_list) > 1) {
    ...
    ggsave(
      ...
      combined, width = 14, height = 6 * length(plot_list), dpi = 300
    )
  }
...
create_performance_plots <- function(performance, output_dir) {
  ...
  # Speed comparison
  p_speed <- ggplot(perf_data, aes(x = sample_size, y = samples_per_second, 
  ...
  # Time scaling
  p_scaling <- ggplot(perf_data, aes(x = sample_size, y = time_seconds, 
                                     color = Generator, group = Generator)) +
    geom_line(size = 1.5) +
    geom_point(size = 3) +
    scale_x_log10(labels = comma) +
  ...
  p_relative <- ggplot(perf_data, aes(x = sample_size, y = relative_speed,
                                      color = Generator, group = Generator)) +
    geom_line(size = 1.5) +
    geom_point(size = 3) +
...
create_category_comparison_plots <- function(suite, output_dir) {
  ...
  ggsave(
    ...
    p_stacked, width = 12, height = 8, dpi = 300
  )
  ...
  if (requireNamespace("ggradar", quietly = TRUE)) {
    ...
    ggsave(
      ...
      p_radar, width = 10, height = 10, dpi = 300
    )
  }
...
create_ranking_plots <- function(suite, output_dir) {
  ...
  # Overall ranking
  p_overall <- ggplot(rankings$overall, aes(x = reorder(Generator, Rank), 
  ...
  p_category <- ggplot(rankings$by_category, 
                      aes(x = Category, y = reorder(Generator, -Rank), 
                          fill = factor(Rank))) +
...
extract_test_matrix <- function(suite) {
  ...
  if (is.null(suite$aggregated)) return(NULL)
...
extract_pvalues <- function(suite) {
  ...
  if (length(pvalue_list) > 0) {
    ...
    return(data.frame(Generator = character(), Test = character(), 
...
prepare_performance_data <- function(performance) {
  ...
  if (length(perf_list) > 0) {
    ...
    return(data.frame(Generator = character(), sample_size = numeric(),
...
extract_category_statistics <- function(suite) {
  ...
  if (length(cat_list) > 0) {
    ...
    return(data.frame(Generator = character(), Category = character(),
...
aggregate_by_category <- function(plot_data) {
  ...
  agg_data <- aggregate(ResultNum ~ Generator + Test, data = plot_data, 
                       FUN = function(x) sum(x, na.rm = TRUE) / length(x))
  names(agg_data)[3] <- "PassRate"
  
  # Extract category from test name
  agg_data$Category <- sapply(strsplit(as.character(agg_data$Test), "_"), 
                             function(x) x[1])
  
  # Final aggregation by category
  cat_data <- aggregate(PassRate ~ Generator + Category, data = agg_data, 
                       FUN = mean, na.rm = TRUE)
