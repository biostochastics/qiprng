...
run_cryptrndtest_suite <- function(x, test_type = "all", alpha = 0.05, ...) {
  ...
  results <- list()
  available_tests <- c("adaptive_chi_square", "birthday_spacings", "book_stack", 
  ...
  if ("adaptive_chi_square" %in% test_type) {
    tryCatch({
      ...
      result <- CryptRndTest::adaptive.chi.square(x_int, B = B, S = S, 
                                                 alpha = alpha)
  ...
  if ("birthday_spacings" %in% test_type) {
    tryCatch({
      ...
      result <- CryptRndTest::birthday.spacings(x_int, m = m, n = n_param,
                                               lambda = lambda_param, 
                                               alpha = alpha)
  ...
  if ("gcd" %in% test_type) {
    tryCatch({
      ...
      result <- CryptRndTest::GCD.test(x_matrix, B = B, mu = mu_param, 
  ...
  if ("random_walk" %in% test_type) {
    tryCatch({
      ...
      results$random_walk <- list(
        ...
        result = ifelse(min(result$Excursion$p.value, 
                           result$Expansion$p.value,
                           result$Height$p.value) >= alpha, "PASS", "FAIL"),
  ...
  if ("topological_binary" %in% test_type) {
    tryCatch({
      ...
      if (length(x_int) >= 8) {
        ...
        result <- CryptRndTest::topological.binary(x_int, B = B, 
                                                  alpha = alpha,
...
run_randtests_suite <- function(x, test_type = "all", alpha = 0.05, ...) {
  ...
  results <- list()
  available_tests <- c("runs", "bartels_rank", "cox_stuart", 
...
run_cryptrndtest_wrapper <- function(x, test_params = list()) {
  ...
  tryCatch({
    ...
    tryCatch({
      ...
      n_param <- 2^16
      lambda_param <- m^3 / (4 * n_param)
      results$birthday_spacings <- birthday.spacings(x_int, m = m, n = n_param,
                                                     lambda = lambda_param)
    }, error = function(e) NULL)
    ...
    tryCatch({
      ...
      critical_value <- floor(length(x_int) * 0.95)
      results$topological_binary <- topological.binary(x_int, B = B, 
                                                       critical.value = critical_value)
    }, error = function(e) NULL)
...
run_multidim_2d_wrapper <- function(x, test_params = list()) {
  ...
  tryCatch({
    ...
    summary_rows <- lapply(names(results), function(test_name) {
      ...
      stat <- if (!is.null(test_result$statistic)) test_result$statistic 
             else if (!is.null(test_result$test_statistic)) test_result$test_statistic
             else NA
...
extract_p_value <- function(test_result) {
  if (is.list(test_result)) {
    if (!is.null(test_result$p.value)) return(test_result$p.value)
    if (!is.null(test_result$p_value)) return(test_result$p_value)
  }
  return(NA)
}
...
extract_statistic <- function(test_result) {
  if (is.list(test_result)) {
    if (!is.null(test_result$statistic)) return(as.numeric(test_result$statistic))
    if (!is.null(test_result$test_statistic)) return(test_result$test_statistic)
  }
  return(NA)
}
