...
benchmark_category <- function(category, config, verbose) {
  ...
  if (length(results$sample_sizes) >= 3) {
    results$complexity <- analyze_complexity(results$sample_sizes, 
...
analyze_scalability <- function(config, verbose) {
  ...
  for (n in sample_sizes) {
    ...
    if (verbose) {
      cat(sprintf("gen=%.3fs, test=%.3fs, total=%.3fs\n", 
  ...
  for (cores in cores_to_test) {
    ...
    if (verbose) {
      ...
      if (cores > 1 && !is.na(baseline_time)) {
        cat(sprintf(" (speedup: %.2fx, efficiency: %.1f%%)", 
...
visualize_performance_results <- function(perf_results, output_dir = "performance_plots") {
  ...
  if (!is.null(exec_data) && nrow(exec_data) > 0) {
    p1 <- ggplot2::ggplot(exec_data, 
                          ggplot2::aes(x = sample_size, y = execution_time, 
                                      color = category)) +
      ggplot2::geom_line() +
      ggplot2::geom_point() +
  ...
  if (!is.null(perf_results$memory_profile$operations)) {
    ...
    p2 <- ggplot2::ggplot(mem_data,
                          ggplot2::aes(x = sample_size, y = operation, 
                                      fill = memory_mb)) +
      ggplot2::geom_tile() +
  ...
  if (!is.null(perf_results$scalability$sample_sizes)) {
    ...
    scale_long <- reshape2::melt(scale_data, id.vars = "sample_size",
                                variable.name = "phase", value.name = "time")
    
    p3 <- ggplot2::ggplot(scale_long,
