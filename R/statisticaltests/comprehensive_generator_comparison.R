...
benchmark_generators <- function(generators, sample_sizes, verbose = TRUE) {
  ...
  for (gen_name in names(generators)) {
    ...
    for (i in seq_along(sample_sizes)) {
      ...
      if (verbose) {
        cat(sprintf("  %s samples: %.3f seconds (%.0f samples/sec)\n",
                   format(n, big.mark = ","),
                   timings[i],
                   n / timings[i]))
      }
