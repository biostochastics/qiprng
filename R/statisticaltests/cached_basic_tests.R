...
run_cached_basic_tests <- function(suite) {
  ...
  chi_squared_func <- function(data, config, ...) {
    ...
    bins <- cut(data, breaks = seq(0, 1, length.out = bins_count + 1), 
                include.lowest = TRUE)
    observed <- table(bins)
    ...
    list(
      ...
      details = paste("Tests uniformity using chi-squared test with", bins_count, "bins.",
                     "p-value should be above significance level for uniformity.")
    )
  }
  ...
  mean_test_func <- function(data, config, ...) {
    ...
    list(
      ...
      details = paste("Tests if mean is close to 0.5.",
  ...
  variance_test_func <- function(data, config, ...) {
    ...
    p_value <- 2 * min(pchisq(chi_statistic, n - 1), 
    ...
    list(
      ...
      details = paste("Tests if variance is close to 1/12.",
