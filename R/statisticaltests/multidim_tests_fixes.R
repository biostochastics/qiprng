...
ripleys_k_test_fixed <- function(points, radii = NULL, config = NULL) {
  ...
  return(list(
    ...
    details = sprintf("L-function max deviation: %.4f at radius %.3f", 
...
scatter_test_3d_fixed <- function(points, method = "triplet") {
  ...
  if (method == "triplet") {
    ...
    # Two-sided p-value
    p_value <- 2 * min(mean(boot_means <= observed_mean),
    ...
    result <- list(
      ...
      details = sprintf("Mean triplet area: %.4f (expected: %.4f ± %.4f)", 
...
nearest_neighbor_test_3d_fixed <- function(points, k_nearest = 1, config = NULL) {
  ...
  return(list(
    ...
    details = sprintf("Mean NN dist: %.4f (expected: %.4f ± %.4f), CV: %.3f", 
