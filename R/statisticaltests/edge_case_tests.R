...
test_data_type_edge_cases <- function(config, verbose = TRUE) {
  ...
  for (type_name in names(edge_types)) {
    ...
    test_result <- tryCatch({
      ...
      if (type_name %in% c("character_values", "logical_values", "factor_values", 
...
test_concurrency_edge_cases <- function(config, verbose = TRUE) {
  ...
  tryCatch({
    ...
    if (all_success) {
      ...
      results$details$parallel <- paste("Failed on workers:", 
  ...
  tryCatch({
    ...
    if (end_count >= 5 && end_count <= expected_calls * 2) {
      ...
      results$failed <- results$failed + 1
      results$details$race_condition <- sprintf("Counter: %d (expected ~%d)", 
...
test_memory_edge_cases <- function(config, verbose = TRUE) {
  ...
  tryCatch({
    ...
    if (mem_increase < 1000) {
      ...
      results$failed <- results$failed + 1
      results$details$large_memory <- sprintf("Excessive memory use: %.1f MB", 
                                             mem_increase)
