...
visualize_binary_tests <- function(suite, bit_matrix, bit_proportions) {
  ...
  p1 <- ggplot2::ggplot(position_data, ggplot2::aes(x = Position, y = Proportion)) +
    ...
    ggplot2::geom_hline(
      yintercept = c(lower_critical, upper_critical),
  ...
  run_length_data_long <- reshape2::melt(run_length_data,
    id.vars = "Length",
    variable.name = "Type",
    value.name = "Count"
  )
  ...
  # Convert to long format for grouped bar chart
  poker_data_long <- reshape2::melt(poker_data,
    id.vars = "Pattern",
    variable.name = "Type",
    value.name = "Count"
  )
