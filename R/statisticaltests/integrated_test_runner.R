...
run_integrated_test_suite <- function(suite, 
  ...
  progress_config <- list(
    ...
    log_file = if (progress_backend == "file") 
      paste0("test_progress_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".log") else NULL
  )
  ...
  if (export_results) {
    export_file <- paste0("test_results_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".", 
...
apply_config_to_suite <- function(suite, config) {
  ...
  suite$config$sample_size <- get_test_param("global.default_sample_size", config, 
                                            suite$config$sample_size)
  suite$config$significance_level <- get_test_param("global.significance_level", config,
                                                   suite$config$significance_level)
  
  # Apply performance settings
  suite$config$parallel <- get_test_param("performance.parallel_enabled", config, FALSE)
  suite$config$max_cores <- get_test_param("performance.max_cores", config, 
...
count_total_tests <- function(suite, config) {
  ...
  if (isTRUE(suite$config$run_compression_tests)) {
    algorithms <- get_test_param("compression_tests.algorithms", config, 
                               c("gzip", "bzip2", "xz"))
