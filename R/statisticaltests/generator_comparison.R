...
compare_all_generators <- function(generators = NULL,
  ...
  cat(sprintf(
    "Comparing %d generators with %d samples\n",
  ...
  generate_unified_report(unified_suite,
    format = "html",
...
plot_generator_comparison <- function(comparison_results, output_dir = NULL) {
  ...
  plots$pass_rate <- ggplot2::ggplot(
    summary_data,
    ggplot2::aes(
      x = reorder(generator, pass_rate),
      y = pass_rate
    )
  ) +
  ...
  plots$test_breakdown <- ggplot2::ggplot(
    test_breakdown,
    ggplot2::aes(
      x = generator,
      y = count,
