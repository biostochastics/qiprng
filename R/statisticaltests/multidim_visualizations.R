...
visualize_2d_comprehensive <- function(points, test_results, output_dir = NULL,
  ...
  p2 <- ggplot(data.frame(x = points[, 1], y = points[, 2]), 
               aes(x = x, y = y)) +
    geom_hex(bins = 30) +
    scale_fill_gradient(low = "lightblue", high = "darkblue") +
    coord_fixed() +
    theme_minimal() +
    labs(title = "2D Hexbin Density Plot",
...
visualize_grid_test_2d <- function(points, grid_result) {
  ...
  p <- ggplot(cell_data, aes(x = x, y = y, fill = deviation)) +
    ...
    coord_fixed() +
    theme_minimal() +
    labs(title = sprintf("Grid Test: Chi-square = %.2f, p = %.3f",
                        grid_result$statistic, grid_result$p_value),
         subtitle = sprintf("%d×%d grid, %d empty cells",
...
visualize_nn_test_2d <- function(points, nn_result) {
  ...
  p <- ggplot(df) +
    ...
    coord_fixed() +
    theme_minimal() +
    labs(title = sprintf("Nearest Neighbor Test: z = %.2f, p = %.3f",
                        nn_result$statistic, nn_result$p_value),
         subtitle = sprintf("Mean distance: %.4f (expected: %.4f)",
...
visualize_ripleys_k <- function(ripley_result) {
  ...
  p <- ggplot(df, aes(x = radius)) +
    ...
    theme_minimal() +
    labs(title = sprintf("Ripley's K Function: KS = %.3f, p = %.3f",
...
create_3d_scatter_interactive <- function(points, test_results) {
  ...
  p <- plotly::plot_ly(
    ...
    text = paste("Point:", 1:nrow(points),
...
create_3d_static_with_projections <- function(points, test_results) {
  ...
  # XY projection
  plot(points[, 1], points[, 2], pch = 19, cex = 0.3,
       main = "XY Projection", xlab = "Dim 1", ylab = "Dim 2",
       xlim = c(0, 1), ylim = c(0, 1))
  grid()
  
  # XZ projection
  plot(points[, 1], points[, 3], pch = 19, cex = 0.3,
       main = "XZ Projection", xlab = "Dim 1", ylab = "Dim 3",
       xlim = c(0, 1), ylim = c(0, 1))
  grid()
  
  # YZ projection
  plot(points[, 2], points[, 3], pch = 19, cex = 0.3,
  ...
  for (i in 1:n_slices) {
    ...
    if (length(slice_idx) > 10) {
      ...
      p <- ggplot(df, aes(x = x, y = y)) +
        ...
        coord_fixed() +
        theme_minimal() +
        labs(title = sprintf("Slice %d: z ∈ [%.2f, %.2f]", 
                            i, z_min, z_max),
...
create_multidim_diagnostic_plot <- function(results_list, output_file = NULL) {
  ...
  p <- ggplot(plot_data, aes(x = test, y = generator, fill = p_value)) +
    ...
    labs(title = "Multi-dimensional Test Results",
...
animate_multidim_evolution <- function(points_list, dimension = 2, 
  ...
  p <- ggplot(all_data, aes(x = x, y = y)) +
    ...
    coord_fixed() +
    theme_minimal() +
    labs(title = "Point Evolution: Time {frame_time}",
