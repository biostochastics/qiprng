...
p <- ggplot(test_pass_data, aes(x = reorder(Test, PassRate), y = PassRate, text = paste0(Test, ": ", round(PassRate, 1), "%"))) +
  ...
  coord_flip() +
  labs(
    title = "Statistical Test Pass Rates",
...
top_performers <- summary_data[top_indices, c(
  "index", "a", "b", "c", "discriminant",
...
display_table <- top_performers[, c(
  "index", "a", "b", "c", "discriminant",
...
dplyr::bind_rows(
  list(
    Uniformity = uniformity_fail,
    Independence = independence_fail,
    Periodicity = periodicity_fail
  ),
  .id = "Test"
) %>%
...
bottom_performers <- non_error_data[bottom_indices, c(
  "index", "a", "b", "c", "discriminant",
  "overall_score", "quality_rating",
  "uniformity_passed", "independence_passed",
  "autocorrelation_passed", "periodicity_passed"
...
display_table_bottom <- bottom_performers[, c(
  "index", "a", "b", "c", "discriminant",
...
datatable(interactive_data,
  ...
  formatRound(columns = intersect(c(
    "overall_score", "ks_p_value", "chi_sq_p_value", "independence_p_value",
    "mean_error", "var_error"
  ), colnames(interactive_data)), digits = 4)
```
...
if (nrow(excellent_discriminants) > 0) {
  ...
  for (i in 1:nrow(top_excellent)) {
    row <- top_excellent[i, ]
    cat(
      "- **Discriminant #", row$index, "**: a=", row$a, ", b=", row$b, ", c=", row$c,
...
if (exists("correlations")) {
  ...
  cat(
    "5. **Parameter Correlation**: The strongest correlation with quality is ",
