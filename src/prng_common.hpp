...
inline bool safe_calculate_discriminant(long a, long b, long c, long long& result,
...
// Method 3: Manual overflow checking (portable fallback)
    ...
    if (a_ll != 0 && c_ll != 0) {
        ...
        if (std::abs(four_a) > 0 && std::abs(c_ll) > LLONG_MAX / std::abs(four_a)) {
            error_msg =
                "Overflow in 4*a*c: 4*a=" + std::to_string(four_a) + ", c=" + std::to_string(c);
            return false;
        }
...
struct PRNGDefaults {
    ...
    enum NormalMethod { BOX_MULLER, ZIGGURAT };
...
class MPFRMemoryPool {
   ...
   public:
    static mpfr_t* allocate() { return instance().pool_.allocate(); }

    static void deallocate(mpfr_t* ptr) { instance().pool_.deallocate(ptr); }

    static size_t available() { return instance().pool_.available(); }
};
...
class MPFRWrapper {
   ...
   public:
    ...
    inline bool is_valid() const noexcept { return initialized_; }
    ...
    MPFRWrapper(MPFRWrapper&& other) noexcept
        ...
        if (other.initialized_) {
            try {
                ...
                if (initialized_) {  // This flag might be true if mpfr_init2 succeeded but mpfr_set
                                     // failed then threw
                    mpfr_clear(value);
                    initialized_ = false;
                }
        ...
        if (this != &other) {
            ...
            if (other.initialized_) {
                try {
                    ...
                    if (ret != 0 || mpfr_nan_p(value) || mpfr_inf_p(value)) {
                        mpfr_clear(
                            value);  // Clean up 'this' if copy failed or resulted in non-finite
                        initialized_ = false;
                    } else {
                        initialized_ = true;  // Successfully copied
                    }
                    ...
                    // Clean up 'this' on error if it was partially initialized
                    if (initialized_) {  // This flag might be true if mpfr_init2 succeeded but
                                         // mpfr_set failed then threw
                        mpfr_clear(value);
                        initialized_ = false;
                    }
    ...
    inline mpfr_prec_t get_precision() const noexcept { return cached_precision_; }
    ...
    inline void add_si(long val) { mpfr_add_si(*get(), *get(), val, MPFR_RNDN); }

    inline void mul_si(long val) { mpfr_mul_si(*get(), *get(), val, MPFR_RNDN); }

    // Fast comparison operations
    inline int compare(const MPFRWrapper& other) const { return mpfr_cmp(*get(), *other.get()); }
...
class SecureBuffer {
   ...
   public:
    ...
    typename std::vector<T, AlignedAllocator<T>>::const_iterator begin() const {
        return data_.begin();
    }
