...
class ParallelJumpAhead {
   public:
    ...
    create_independent_streams(const QuadraticIrrational& base_qi, size_t num_streams,
        ...
        const size_t num_threads =
            std::min(num_streams - 1, size_t(std::thread::hardware_concurrency()));
        ...
        if (num_threads > 1 && num_streams > 2) {
            ...
            for (size_t i = 1; i < num_streams; ++i) {
                futures.push_back(std::async(std::launch::async, [&base_qi, i, jump_distance]() {
    ...
    create_independent_ensembles(const std::vector<std::tuple<long, long, long>>& base_params,
                                 int mpfr_precision, size_t num_ensembles,
                                 uint64_t jump_distance = 1000000,
        ...
        const size_t num_threads =
            std::min(num_ensembles, size_t(std::thread::hardware_concurrency()));
        ...
        if (num_threads > 1) {
            ...
            for (size_t i = 0; i < num_ensembles; ++i) {
                futures.push_back(std::async(std::launch::async, [base_params, mpfr_precision, i,
                                                                  jump_distance, strategy]() {
                    // Create ensemble with unique seed based on index
                    auto ensemble = std::make_unique<MultiQI>(
                        base_params, mpfr_precision, i * 12345, true,  // Unique seed per ensemble
                        strategy);
            ...
            // Sequential creation
            for (size_t i = 0; i < num_ensembles; ++i) {
                auto ensemble = std::make_unique<MultiQI>(base_params, mpfr_precision, i * 12345,
                                                          true, strategy);
    ...
    static void parallel_fill_with_streams(double* buffer, size_t buffer_size,
                                           std::vector<std::unique_ptr<MultiQI>>& ensembles) {
        if (ensembles.empty() || buffer_size == 0)
            return;
        ...
        for (size_t i = 0; i < num_streams; ++i) {
            ...
            futures.push_back(
                std::async(std::launch::async, [&ensembles, i, buffer, start, size]() {
    ...
    static void parallel_fill_with_work_stealing(double* buffer, size_t buffer_size,
        ...
        const size_t num_chunks = std::max(size_t(num_threads * 4), buffer_size / min_chunk);
        ...
        for (size_t i = 0; i < num_chunks && current_pos < buffer_size; ++i) {
            ...
            if (i % 3 == 0)
                chunk_size *= 2;  // Some chunks are larger
            if (i % 5 == 0)
                chunk_size *= 3;  // Some are even larger
