    ...
    for (size_t i = 0; i < num_qis; ++i) {
        ...
        if (disc > 0) {
            ...
            if (QuadraticIrrational::is_square_free(disc)) {
                qis_.push_back(std::make_unique<QuadraticIrrational>(a, b, c, mpfr_prec, seed + i, has_seed));
            } else {
                ...
                qis_.push_back(std::make_unique<QuadraticIrrational>(1, p, -1, mpfr_prec, seed + i, has_seed));
            }
        } else {
            // Fallback to a known good configuration with square-free discriminant = 5
            qis_.push_back(std::make_unique<QuadraticIrrational>(1, 1, -1, mpfr_prec, seed + i, has_seed));
        }
    }
    ...
    try {
        ...
        if (use_simd && qis_.size() >= 2) {
            ...
            for (size_t batch = 0; batch < num_batches; ++batch) {
                ...
                // Use SIMD XOR mixing
                simd::xor_mix_batch(buffer + batch * batch_size,
                                   temp1.data(), temp2.data(), batch_size);
            }
            ...
            if (mixing_strategy_ == MixingStrategy::ROUND_ROBIN) {
                ...
                for (size_t i = 0; i < fill_size; i++) {
                    ...
                    #ifdef DEBUG_MIXING
                    if (i == 0) {
                        Rcpp::Rcout << "DEBUG: mix_size=" << mix_size << ", idx_=" << idx_ << std::endl;
                    }
                    #endif
...
void MultiQI::fill_parallel(double* buffer, size_t fill_size) {
    ...
    if (fill_size >= 10000 && chunk_size >= 1000) {
        ...
        #pragma omp parallel
        {
            ...
            #pragma omp critical
            {
                thread_qis[tid] = std::make_unique<MultiQI>(
                    abc_list, 256, tid * 1000000, true, mixing_strategy_
                );
            }
        }
