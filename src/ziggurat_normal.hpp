...
class ZigguratNormal {
   private:
    ...
    static std::array<uint32_t, ZIGGURAT_TABLE_SIZE>
        cached_k_table_;                      // scaled x_table_[i+1]/x_table_[i]
   ...
   public:
    ZigguratNormal(std::function<double()> uniform_generator, double mean = 0.0,
                   double stddev = 1.0, bool thread_safe_mode = false);
...
class ZigguratTLSManager {
   private:
    ...
    ZigguratTLSManager()
        : valid_(true), owner_id_(nullptr), thread_id_(std::this_thread::get_id()),
   ...
   public:
    ...
    bool is_valid() const {
        ...
        return valid_.load(std::memory_order_acquire) &&
               !ZigguratNormal::is_cleanup_in_progress() && !thread_exiting_;
    }

    // Mark thread as exiting
    static void mark_thread_exiting() { thread_exiting_ = true; }

    // Check if thread is exiting
    static bool is_thread_exiting() { return thread_exiting_; }
