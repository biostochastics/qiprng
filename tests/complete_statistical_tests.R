...
tryCatch({
  ...
  record_result("Basic", "Deterministic", 
...
tryCatch({
  ...
  for (lag in 1:10) {
    ...
    if (lag < n) {
      correlations[lag] <- cor(test_sequence[1:(n-lag)], 
  ...
  record_result("Independence", "SerialCorrelation", status, 
...
tryCatch({
  ...
  record_result("Independence", "RunsTest", status,
                list(n_runs = n_runs, expected = expected_runs, 
...
tryCatch({
  ...
  status <- ifelse(p_value > 0.01, "PASSED", "MARGINAL")
  record_result("Uniformity", "ChiSquared", status,
...
tryCatch({
  if (requireNamespace("nortest", quietly = TRUE)) {
    ...
    record_result("Uniformity", "AndersonDarling", status,
...
tryCatch({
  ...
  record_result("Performance", "ParallelSpeedup", "MEASURED",
...
tryCatch({
  ...
  record_result("SpecialFeatures", "JumpAhead", status,
...
tryCatch({
  ...
  record_result("SpecialFeatures", "MultiQI", status,
