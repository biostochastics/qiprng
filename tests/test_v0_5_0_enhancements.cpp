...
void test_xor_mixing() {
    ...
    std::vector<std::tuple<long, long, long>> abc_list = {{1, 5, -2}, {2, 7, -3}};
...
void test_matrix_power() {
    ...
    std::cout << "  Binary exp result: [" << m10.p << ", " << m10.q << ", " << m10.r << ", "
              << m10.s << "]\n";
    std::cout << "  Manual result:     [" << manual.p << ", " << manual.q << ", " << manual.r
              << ", " << manual.s << "]\n";

    assert(m10.p == manual.p && m10.q == manual.q && m10.r == manual.r && m10.s == manual.s);
...
void test_mixing_strategies() {
    ...
    std::vector<std::tuple<long, long, long>> abc_list = {{1, 5, -2}, {2, 7, -3}, {3, 11, -5}};

    MixingStrategy strategies[] = {MixingStrategy::ROUND_ROBIN, MixingStrategy::XOR_MIX,
                                   MixingStrategy::AVERAGING, MixingStrategy::MODULAR_ADD,
                                   MixingStrategy::CASCADE_MIX};

    const char* names[] = {"ROUND_ROBIN", "XOR_MIX", "AVERAGING", "MODULAR_ADD", "CASCADE_MIX"};
    ...
    for (int s = 0; s < 5; s++) {
        ...
        for (int i = 0; i < 1000; i++) {
            double val =
                (strategies[s] == MixingStrategy::ROUND_ROBIN) ? multi.next() : multi.next_mixed();
            sum += val;
        ...
        std::cout << "  " << names[s] << ":\n";
        std::cout << "    Mean: " << (sum / 1000.0) << ", Range: [" << min_val << ", " << max_val
                  << "]\n";
    }
