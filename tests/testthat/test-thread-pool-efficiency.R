...
test_that("Thread pool optimization improves parallel performance", {
  ...
  # Display results
  cat(sprintf("No threading:          %.4f seconds (mean=%.4f, sd=%.4f)\n", 
              result1$time, result1$mean, result1$sd))
  cat(sprintf("Threading only:        %.4f seconds (mean=%.4f, sd=%.4f)\n", 
              result2$time, result2$mean, result2$sd))
  cat(sprintf("Threading + parallel:  %.4f seconds (mean=%.4f, sd=%.4f)\n", 
...
test_that("Thread-safe Ziggurat comparison with Box-Muller", {
  ...
  # Display results
  cat(sprintf("Ziggurat method:   %.4f seconds (mean=%.4f, sd=%.4f)\n", 
              ziggurat_result$time, ziggurat_result$mean, ziggurat_result$sd))
  cat(sprintf("Box-Muller method: %.4f seconds (mean=%.4f, sd=%.4f)\n", 
