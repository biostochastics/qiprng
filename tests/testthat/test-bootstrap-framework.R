...
test_that("bootstrap_p_value works correctly", {
  ...
  result2 <- bootstrap_p_value(x2, stat_func2, n_bootstraps = 1000, 
...
test_that("bootstrap_p_value alternative hypotheses work", {
  ...
  # Two-sided test
  result_two <- bootstrap_p_value(x, stat_func, observed_stat = obs_skew,
                                   alternative = "two.sided",
                                   n_bootstraps = 1000, seed = 456)
  
  # One-sided tests
  result_greater <- bootstrap_p_value(x, stat_func, observed_stat = obs_skew,
                                      alternative = "greater",
                                      n_bootstraps = 1000, seed = 456)
  result_less <- bootstrap_p_value(x, stat_func, observed_stat = obs_skew,
                                   alternative = "less",
                                   n_bootstraps = 1000, seed = 456)
...
test_that("permutation_test works for one-sample tests", {
  ...
  result <- permutation_test(x, test_statistic = stat_func,
...
test_that("permutation_test handles errors gracefully", {
  # Invalid inputs
  expect_error(permutation_test("not numeric", test_statistic = mean),
               "must be numeric")
  expect_error(permutation_test(1:10, "not numeric", test_statistic = mean),
               "must be numeric")
  expect_error(permutation_test(1:10, test_statistic = "not_function"),
               "must be a function")
})
...
test_that("monte_carlo_p_value validates inputs", {
  ...
  expect_error(monte_carlo_p_value("not numeric", null_sim, stat_func),
               "single numeric value")
  expect_error(monte_carlo_p_value(c(1, 2), null_sim, stat_func),
               "single numeric value")
  
  # Invalid functions
  expect_error(monte_carlo_p_value(0, "not function", stat_func),
               "must be a function")
  expect_error(monte_carlo_p_value(0, null_sim, "not function"),
               "must be a function")
  
  # Too few simulations
  expect_error(monte_carlo_p_value(0, null_sim, stat_func, n_simulations = 50),
               ">= 100")
})
...
test_that("bootstrap_conf_interval works correctly", {
  ...
  result <- bootstrap_conf_interval(x, mean, n_bootstraps = 1000,
...
test_that("progress bars work correctly", {
  ...
  result1 <- bootstrap_p_value(x, mean, n_bootstraps = 1000,
                               progress = TRUE, seed = 404)
  expect_true(is.list(result1))
  
  # Should not show progress bar
  result2 <- bootstrap_p_value(x, mean, n_bootstraps = 500,
                               progress = TRUE, seed = 404)
  expect_true(is.list(result2))
  
  # Progress = FALSE
  result3 <- bootstrap_p_value(x, mean, n_bootstraps = 2000,
