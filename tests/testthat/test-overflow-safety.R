  ...
  tryCatch(
    {
      ...
      cleanup_prng()
      cat(sprintf(
        "  Case %d (a=%d, b=%d, c=%d): PASSED\n",
        i, cfg$a, cfg$b, cfg$c
      ))
    },
    error = function(e) {
      cat(sprintf(
        "  Case %d (a=%d, b=%d, c=%d): FAILED - %s\n",
  ...
  tryCatch(
    {
      ...
      cleanup_prng()
      cat(sprintf(
        "  Case %d (a=%d, b=%d, c=%d): FAILED - Should have been rejected\n",
        i, cfg$a, cfg$b, cfg$c
      ))
    },
    error = function(e) {
      if (grepl("discriminant", e$message, ignore.case = TRUE)) {
        cat(sprintf(
          "  Case %d (a=%d, b=%d, c=%d): CORRECTLY REJECTED\n",
          i, cfg$a, cfg$b, cfg$c
        ))
      } else {
        cat(sprintf(
          "  Case %d (a=%d, b=%d, c=%d): ERROR - %s\n",
          i, cfg$a, cfg$b, cfg$c, e$message
        ))
      }
    }
  ...
  tryCatch(
    {
      ...
      cleanup_prng()
      cat(sprintf(
        "  Case %d (a=%d, b=%d, c=%d, disc=%d): PASSED\n",
    ...
    error = function(e) {
      if (expected_disc <= 0) {
        cat(sprintf(
          "  Case %d (a=%d, b=%d, c=%d, disc=%d): CORRECTLY REJECTED\n",
          i, cfg$a, cfg$b, cfg$c, expected_disc
        ))
      } else {
        cat(sprintf(
          "  Case %d (a=%d, b=%d, c=%d, disc=%d): FAILED - %s\n",
...
if (.Platform$OS.type != "windows") {
  ...
  for (res in results) {
    if (res$status == "SUCCESS") {
      cat(sprintf(
        "  Thread %d: SUCCESS (generated %d values)\n",
