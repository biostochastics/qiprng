...
test_that("p-value distribution follows uniform distribution (KS test)", {
  ...
  expect_true(ks_result$p.value > 0.01,
    info = paste(
      "KS test p-value:", round(ks_result$p.value, 4),
      "- P-values should be uniformly distributed"
    )
  )
  ...
  expect_true(abs(mean(p_values) - 0.5) < 0.2,
    info = paste(
      "Mean p-value:", round(mean(p_values), 3),
