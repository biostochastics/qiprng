...
void thread_function(int thread_id, std::atomic<int>& success_count, std::atomic<int>& failure_count) {
    try {
...
int main() {
    ...
    for (int iter = 0; iter < NUM_ITERATIONS; ++iter) {
        ...
        for (int i = 0; i < NUM_THREADS; ++i) {
            threads.emplace_back(thread_function, i, std::ref(success_count), std::ref(failure_count));
        }
        ...
        // Report results
        std::cout << "Results: " << success_count.load() << " successes, " 
    ...
    std::cout << "The std::call_once fix properly addresses the double-checked locking issue." << std::endl;
