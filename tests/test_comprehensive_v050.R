...
run_test("All Distributions", function() {
  distributions <- list(
    ...
    list(
      name = "uniform_range", range_min = -5, range_max = 5,
      check = function(x) all(x >= -5 & x <= 5)
    ),
    list(
      name = "normal", normal_mean = 10, normal_sd = 2,
      check = function(x) abs(mean(x) - 10) < 0.5
    ),
    list(
      name = "exponential", exponential_lambda = 2,
      check = function(x) all(x >= 0)
    ),
    list(
      name = "poisson", poisson_lambda = 5,
      check = function(x) all(x >= 0 & x == floor(x))
    ),
    list(
      name = "bernoulli", bernoulli_p = 0.3,
