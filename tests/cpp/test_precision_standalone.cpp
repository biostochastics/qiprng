...
void test_pi_precision() {
    ...
    std::cout << "MPFR extended: +" << (digits_extended - digits_hardcoded) << " digits"
              << std::endl;
...
void test_conversion_precision() {
    ...
    if (error_safe <= error_direct) {
        ...
        std::cout << "✓ Extended conversion reduces error by " << std::fixed << std::setprecision(1)
...
void test_critical_values() {
    ...
    for (const auto& test : cases) {
        ...
        std::cout << std::left << std::setw(10) << test.name << " Direct: " << digits_direct
                  << " digits, "
