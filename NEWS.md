...
### New cache_optimized.hpp Header

* **Cache-line aligned structures**: Implemented `PackedQIState` struct aligned to 64 bytes
  * Packs all data for one QI iteration into a single cache line
  * Eliminates cache line straddling for improved memory access
  * Platform-portable alignment using `alignas(CACHE_LINE_BYTES)`
* **Prefetch utilities**: Added cross-platform prefetch hint functions
  * `prefetch_read()`: Brings data into cache for reading
  * `prefetch_write()`: Hints for upcoming write operations
  * `prefetch_ahead()`: Template for prefetching array elements
  * GCC/Clang `__builtin_prefetch` with MSVC `_mm_prefetch` fallback
* **Aligned memory allocation**: Platform-specific aligned allocator
  * POSIX `posix_memalign()` for Unix/macOS
  * Windows `_aligned_malloc()` for MSVC
* **Batch processor**: `CacheOptimizedBatchProcessor` class
  * Contiguous state storage with cache line separation
  * Loop unrolling for instruction-level parallelism
  * Integrated prefetch hints for latency hiding

### Hot Loop Optimizations

* **QuadraticIrrational::fill()**: Added prefetch hints for write buffer
  * Prefetches 4 cache lines ahead during batch generation
  * Reduces memory stalls in hot loops
* **MultiQI::refillCache()**: Integrated cache optimization
  * Uses `cache::OPTIMAL_BATCH_SIZE` for L1-friendly working sets
  * Added conditional prefetching based on buffer size threshold
* **Expected impact**: 2-3x throughput improvement for batch operations

### Platform Portability

* **MSVC support**: Added `<xmmintrin.h>` include with `QIPRNG_HAS_MSVC_PREFETCH` guard
* **Windows compatibility**: Platform-specific `<malloc.h>` for aligned allocation
* **Compile-time verification**: Static assertions ensure:
  * Cache line size is power of two
  * `PackedQIState` exactly fills one cache line
